...
class UIComponents:
    ...
    def create_upper_section(self, show_offer_number=False):
        ...
        self.entries['address1'] = Entry(self.window, width=28, textvariable=address1_value)
        self.entries['address1'].place(x=110, y=118)

        address2_value = StringVar(self.window, value=self.text_data['address_2'])
        self.entries['address2'] = Entry(self.window, width=28, textvariable=address2_value)
        ...
        self.entries['phone'] = Entry(self.window, width=18, textvariable=phone_value)
    ...
    def load_default_supplier(self):
        ...
        try:
            ...
            # If error occurs, supplier fields remain empty (which is the desired fallback)
